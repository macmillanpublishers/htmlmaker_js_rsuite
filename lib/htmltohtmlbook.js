...
var idCounter = 0;
// a global var to track errors in container tagging
var errorstring = ""
...
containerparas['box'].forEach(function ( val ) {
  $( val ).each(function() {
    ...
    if (containerendparas.indexOf("."+containerend_class) != -1) {
      ...
      errorstring += "!container not ended "
    }
    $(".temp").removeClass("temp");
...
function makeSubLists(sublistparas, sublisttype, deepersublistparas = []) {
...
makeSubLists(unorderedlistparaslevel2, "ul", level3listparalist);
makeSubLists(orderedlistparaslevel2, "ol", level3listparalist);

makeSubLists(unorderedlistparaslevel3, "ul");
makeSubLists(orderedlistparaslevel3, "ol");
