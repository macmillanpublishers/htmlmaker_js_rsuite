...
containerparas['computer'].forEach(function ( val ) {
  $( val ).each(function() {
    var nextsiblings = $(this).nextUntil(containerandsectionparaslist).addBack();
    // skip to next selection if no elements in container:
    //  empty container creates issue with prince due to self-closing tag
    console.log("computer")
    console.log(nextsiblings.length)
    if (nextsiblings.length == 1) {
      console.log("found an empty container!");
      return true;
    }
...
containerparas['table'].forEach(function ( val ) {
  $( val ).each(function() {
    ...
    //  empty container creates issue with prince due to self-closing tag
    console.log("table")
    console.log(nextsiblings.length + nextsiblings)
    if (nextsiblings.length == 1) {
      console.log("found an empty container!");
      nextsiblings.first().remove(); // remove container-start para
      return true;
...
// <figure class="Illustrationholderill" id="fig-id16by3155b"><img src="images/dragons-fantasy_fullpage.jpg" alt="This%20is%20a%20caption%20above%20an%20image."/>
//     <figcaption class="Captioncap" id="idqe6ldmpfx">This is a caption above an image.</figcaption>
//     <p class="IllustrationSourceis" id="idns8fuoh9q"><a class="fig-link">Image courtesy of some great wizard</a></p>
// </figure>
