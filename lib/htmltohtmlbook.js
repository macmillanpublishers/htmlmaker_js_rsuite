...
function makeNot(list) {
  return "body:not(" + list + "), section:not(" + list + "), div:not(" + list + "), blockquote:not(" + list + "), h1:not(" + list + "), pre:not(" + list + "), aside:not(" + list + "), p:not(" + list + "), li:not(" + list + "), figure:not(" + list + "), figcaption:not(" + list + "), img:not(" + list + ")";
}
...
containerparas['computer'].forEach(function ( val ) {
  $( val ).each(function() {
    ...
    var newparent = $("<div/>").addClass("computer").addClass("temp");
    $(this).before(newparent);
    var node = $(".temp");
    node.append(nextsiblings);
    var containerend_class = $(".temp").next().attr("class");
...
var all_endnoteparas_selector = $(endnotelist + ", p.nonEndnoteTxt");
