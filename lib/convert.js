		...
		for (var i = 0; para.children && i < para.children.length; i++) {
			...
			for (var j = 0; child.children && j < child.children.length; j++) {
				...
				if (data.type == "text") {
					...
					//   these angle brackets for those runs the &lt; encodings are interpreted as text instead of encodings
...
Convert.prototype.convertDocx = function (stylesJson, docx, docxStyles) {
  ...
  $('span.EndnoteReference').each(function () {
  	...
  	var notenumber = $(this).contents().text();
    // 12-20: added text ^ and related conditional \/, to avoid adding empty refs &/or id's with whitespace
    //  not rm'ing them to avoid rm'ing _any_ content in initial xform; + there is no css for them so they have no impact
    if ((/\s/.test(notenumber) == false) && (notenumber != '')) {   //  < tests for any whitespace, verifies there is a notenumber
