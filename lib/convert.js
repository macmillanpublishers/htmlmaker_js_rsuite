...
var Convert = function (styleFunctionsPath) {
	...
	var $ = cheerio.load('<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body data-type="book"></body></html>');
	this.document = $;
	this.head = $("head");
	this.body = $("body");
	...
	this.aggregateParaText = function (para) {
		...
		for (var i = 0; para.children && i < para.children.length; i++) {
			...
			for (var j = 0; child.children && j < child.children.length; j++) {
				...
				} else if ((data.type == "break" && data.breakType == "line") || (data.type == "lineBreak")) {
					isLineBreak = true;
...
Convert.prototype.convertDocx = function (stylesJson, docx, docxStyles) {
		...
		if (note.noteType == "endnote") {
			...
			note.body.forEach(function (para, index, paras) {
				...
				if ((para.children[0] && para.children[0].styleId == "EndnoteReference") || noteref_found == true) {
					...
					non_noteparent.append(p);
					nnp_counter += 1;
				}
			});
			// If there is no styled endnote reference, we must assume that the whole note-body is note content
			if (np_counter == 0 && nnp_counter > 0) {
				non_noteparent.addClass('endnotetext');
				non_noteparent.attr("id", endnoteid);
				convert.body.append(non_noteparent);
			// If content preceded the endnote ref, we can assume it is non-endnote text
			} else if (np_counter > 0 && nnp_counter > 0) {
				non_noteparent.addClass('nonEndnoteTxt');
				convert.body.append(non_noteparent);
	...
	return this.document.html({ xmlMode:true });
};
