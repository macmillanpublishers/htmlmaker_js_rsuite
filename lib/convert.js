	...
	this.aggregateParaText = function (para) {
		...
		for (var i = 0; para.children && i < para.children.length; i++) {
			...
			if (isNoteWithCustomMark) {
				...
				span.text(paraText);
				text.push(span);
			// in testing endnotes, noticed that an unstyled note could go untagged... this remedies that
...
Convert.prototype.convertDocx = function (stylesJson, docx, docxStyles) {
	...
	notesjsonarr.forEach(function (note, index, paras) {
		if (note.noteType == "endnote") {
			// \/ conditional below skips continuation notice endnote, which is otherwise unhandleable via mammoth.js
			//  harmless except that it creates a non-empty notes section, that is not user removeable.
			//  this is to prevent that.
			if (note.body[0]["children"].length > 0){
